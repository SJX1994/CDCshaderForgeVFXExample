...
namespace DigitalRuby.LightningBolt
{
    ...
    public class LightningBoltScriptV2 : MonoBehaviour
    {
        [SerializeField]
        private bool isAroundCloud;
        [SerializeField]
        ...
        private void Start()
        {
            if (isAroundCloud)
            {
                ...
                // StartPosition = new Vector3(StartPosition.x, StartPosition.y + parentPosition.y, StartPosition.z);
                // EndPosition = new Vector3(EndPosition.x, EndPosition.y + parentPosition.y, EndPosition.z);
                Generations = 6;
                Duration = 0.04f;
                ChaosFactor = 0.61f;
                Rows = 1;
                ...
                lr.material.SetColor("_TintColor", lightingColor);
                lr.material.mainTexture = lightingTex;
                curve.AddKey(0, 0);
                curve.AddKey(.1f, .05f);
                curve.AddKey(.9f, .02f);
                curve.AddKey(1, 0);
                transform.GetComponent<LineRenderer>().widthCurve = curve;
